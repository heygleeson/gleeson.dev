...
function ImageLoadBar_hook(ctx, width, height, total, current, image) {
	...
	// get parameters:
	var backgroundColor 	= getc("background_color", "#000");
	var barBackgroundColor 	= getc("bar_background_color", "#088");
	var barForegroundColor 	= getc("bar_foreground_color", "#0FF");
	var barBorderColor 		= getc("bar_border_color", "#FFF");
	var barWidth 			= getf("bar_width", Math.round(width * 0.5));
	var barHeight 			= getf("bar_height", 32);
	var barBorderWidth 		= getf("bar_border_width", 4);
	var barOffset 			= getf("bar_offset", 32);
